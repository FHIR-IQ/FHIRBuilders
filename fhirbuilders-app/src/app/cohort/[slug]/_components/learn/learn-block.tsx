...
// Shared Study-Guide block renderer (Sessions 2+). `sessionKey` namespaces the
// localStorage try-it state so each session's progress is independent.
const STORAGE_KEY = (slug: string, sessionKey: string, blockId: string) =>
  `cohort:${slug}:learn:${sessionKey}:${blockId}`;

type Props = { block: CurriculumBlock; cohortSlug: string; sessionKey: string };

export function LearnBlock({ block, cohortSlug, sessionKey }: Props) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(STORAGE_KEY(cohortSlug, sessionKey, block.id));
      setDone(raw === "true");
    } catch {}
    setHydrated(true);
  }, [cohortSlug, sessionKey, block.id]);
  ...
  function toggle() {
    ...
    setDone(next);
    try {
      localStorage.setItem(STORAGE_KEY(cohortSlug, sessionKey, block.id), String(next));
    } catch {}
  }
          ...
          <div className="space-y-4">
            {block.examples.map((ex, i) => (
              <div key={i}>
                ...
                {ex.note && <p className="mt-1.5 text-xs italic text-slate-500">{ex.note}</p>}
              </div>
            ))}
          </div>
        </div>

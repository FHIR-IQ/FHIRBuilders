...
type Props = { blocks: NavBlock[]; cohortSlug: string; sessionKey: string };

export function BlockNav({ blocks, cohortSlug, sessionKey }: Props) {
  ...
  useEffect(() => {
    function countDone() {
      ...
      try {
        blocks.forEach((b) => {
          if (localStorage.getItem(STORAGE_KEY(cohortSlug, sessionKey, b.id)) === "true") count++;
        });
      } catch {}
      setDoneCount(count);
    }
